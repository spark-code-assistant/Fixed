...
async def Krypton_stats():
  while True:
    ...
    sv_ch = client.get_channel(1202955801614876732)
    users_ch = client.get_channel(1202955821533765632)
...
@tree.command(name="embed", description="Create A Embed Using ForzaNebulla")
...
@app.route('/')
def home():
  return "https://dsc.gg/sparkcodez"
...
async def on_command_completion(context: Context) -> None:
  ...
  hacker = client.get_channel(1189970118030139533)
  if context.guild is not None:
    ...
    try:
      ...
      embed1.add_field(name=":arrow_forward:  Command Name :",
                       value=f"{executed_command}",
                       inline=False)
      embed1.add_field(
        name=":arrow_forward:  Command Executed By :",
        ...
        f"{context.author} | ID: [{context.author.id}](https://discord.com/users/{context.author.id})",
        inline=False)
      embed1.set_footer(text="Powered By ForzaNebulla",
                        icon_url=client.user.display_avatar.url)
